...
def test_resolve_target_schema_found() -> None:
    ...
    assert ref == TargetRef(kind=ObjectKind.schema, schema="public", name=None, oid=None)
...
def test_resolve_target_relation_kinds(relkind: str, expected_kind: ObjectKind) -> None:
    ...
    assert ref == TargetRef(
        kind=expected_kind, schema="public", name="users", oid=42
    )
...
def test_fetch_columns_postgres() -> None:
    ...
    assert cols == [
        ColumnInfo(1, "id", "bigint", False, "nextval('users_id_seq')", True, None, None, None),
...
def test_fetch_columns_postgres_identity_default() -> None:
    ...
    rows = [
        (1, "id", "bigint", False, "GENERATED ALWAYS AS IDENTITY", True, None, None, None),
        (2, "org_id", "bigint", False, "GENERATED BY DEFAULT AS IDENTITY", False, None, None, None),
        (3, "legacy_id", "bigint", False, "nextval('legacy_seq'::regclass)", False, None, None, None),
    ]
    cursor = FakeCursor([rows])
    cols = fetch_columns(cursor, 42, SqlEngine.postgresql)
...
def test_fetch_constraints_parses_pk_fk_unique_check() -> None:
    rows = [
        ...
        ("users_email_uq", "u", "UNIQUE (email)", ["email"], None, None, None, None, None),
...
def test_fetch_indexes() -> None:
    rows = [
        ...
        ("users_active_idx", ["email"], False, False, "btree", 4096, "(active IS TRUE)"),
    ]
    cursor = FakeCursor([rows])
...
def test_fetch_relation_header() -> None:
    ...
    assert hdr == RelationHeader(
        schema="public", name="users", owner="dbadmin",
        tablespace="pg_default", comment="App users",
        row_estimate=12345, total_size=1048576,
        table_size=524288, index_size=262144, toast_size=262144,
    )
...
def test_fetch_policies_postgres() -> None:
    cursor = FakeCursor([
        (True,),  # relrowsecurity
        [("tenant_isolation", "ALL", ["app"],
          "(tenant_id = current_setting('app.tenant')::int)", None)],
    ])
    policies, enabled = fetch_policies(cursor, 42, SqlEngine.postgresql)
    assert enabled is True
    assert policies == [
        PolicyInfo("tenant_isolation", "ALL", ["app"],
                   "(tenant_id = current_setting('app.tenant')::int)", None),
    ]
...
def test_fetch_partitioning_returns_empty_on_redshift() -> None:
    ...
    assert result == PartitioningInfo(parent=None, strategy=None, partition_key=None, children=[])
    assert cursor.queries == []
...
def test_fetch_dependencies_upstream() -> None:
    ...
    deps = fetch_dependencies(cursor, 42, direction="upstream", engine=SqlEngine.postgresql)
    assert deps == [
...
def test_fetch_dependencies_downstream() -> None:
    ...
    deps = fetch_dependencies(cursor, 42, direction="downstream", engine=SqlEngine.postgresql)
...
def test_fetch_dependencies_invalid_direction() -> None:
    cursor = FakeCursor()
    with pytest.raises(ValueError):
        fetch_dependencies(cursor, 42, direction="sideways", engine=SqlEngine.postgresql)


def test_fetch_dependencies_redshift_returns_empty() -> None:
    cursor = FakeCursor()
    assert fetch_dependencies(cursor, 42, direction="upstream", engine=SqlEngine.redshift) == []
    assert fetch_dependencies(cursor, 42, direction="downstream", engine=SqlEngine.redshift) == []
    assert cursor.queries == []
...
def test_fetch_redshift_distribution() -> None:
    ...
    assert d == RedshiftDistribution("KEY", "user_id", "COMPOUND", ["created_at", "user_id"])
...
def test_describe_table_composes_all_sections_postgres() -> None:
    cursor = FakeCursor([
        # fetch_relation_header
        ("public", "users", "dbadmin", "pg_default", None, 100, 1024, 512, 256, 256),
        # fetch_columns
...
def test_describe_view_composes_postgres() -> None:
    cursor = FakeCursor([
        # fetch_relation_header
        ("public", "user_view", "dbadmin", "pg_default", "View over users",
...
def test_describe_table_redshift_populates_distribution_and_stats() -> None:
    ...
    cursor = FakeCursor([
        ...
        ("public", "events", "dbadmin", None, None, 5000, 1048576, None, None, None),
